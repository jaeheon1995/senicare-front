...
const PATCH_NURSE_API = `${NURSE_MODUEL_URL}`
...
// function: pathch nurse 요청 함수 //
export const PatchNurseRequest = async (requestBody:PatchNurseRequestDto,accessToken: string) => {
    const responseBody = await axios.patch(PATCH_NURSE_API,requestBody,bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
};
